...
def run(eff, text=False, graph=False, output="default", layout="five", number=1):
	with open("output/" + output + ".csv", 'wb') as csvout:
		...
		writer.writerow(["Event Number", "Category", "Xpos", "Ypos", "Sig Count", "Background Count", "Illuminated Area"])
		
		for i in range (0, int(number)):
			if graph:
				fig = plt.figure()
	
			rayxpos, rayypos, epsilon, rayradius, Energy, major, minor, ra, rp, e, Z = g.run(text=text)
			...
			with open("orientations/"+ layout +".csv", 'rb') as csvfile:
				...
				for row in reader:
					...
					writer.writerow([int(i+1), category, xpos, ypos, sigcount, bkgcount, litarea])					

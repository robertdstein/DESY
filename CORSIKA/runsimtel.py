...
base_file_name = os.path.join(run_dir, "run" + str(cfg.runnumber) + "_off" + offset)

# open inputcard to extract source theta
f_in = open(os.path.join(run_dir, "run" + str(cfg.runnumber) + ".inputcard"))
for zeile in f_in:
	if "THETAP" in zeile:
...
os.system(". read_hess -S -r 2 -p  run"+str(cfg.runnumber)+"_off0.ps run"+str(cfg.runnumber)+"_off0.simhess.gz >run"+str(cfg.runnumber)+"_read_hess_output.txt")

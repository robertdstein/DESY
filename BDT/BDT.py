...
def run(trainingset, source, detectorcount, mindetections):
	...
	for k in range (detectorcount, mindetections -1, -1):
		...
		with open("/d6/rstein/Hamburg-Cosmic-Rays/positioning/reconstructeddata/"+ str(trainingset) +".csv", 'rb') as csvfile:
		...
		clf = ensemble.GradientBoostingClassifier(max_depth=5, n_estimators=100, learning_rate=1)
		...
		plt.savefig("/d6/rstein/Hamburg-Cosmic-Rays/positioning/graphs/BDTresponse" + str(k) + ".pdf")
	...
	with open("/d6/rstein/Hamburg-Cosmic-Rays/positioning/reconstructeddata/" + str(source) + "_BDT.csv", 'wb') as csvout:
				...
				for row in reader:
					if j < 0:
						...
						if int(detections) == int(k):
							...
							regressor = rgr.predict([entry])[0]
						
							row.append(regressor)
							row.append(BDT)
							writer.writerow(row)

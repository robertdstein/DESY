...
The candidates were checked against the true DC pixels identified in the EAS-free images. From the testing sample, 1.8\% of all images were correctly identified and passed the cuts. Once misidentified events were considered, the post-cuts sample was 89.9\% accurate, as shown in Figure \ref{fig:cutdistribution}. These values served as a benchmark for BDT performance.
...
\begin{table}[h!]
  ...
  \begin{tabular}{ccc}
    ...
    & $Q_{DC}$ & BDT\\
    \midrule
    Pixels Accepted and Correctly Identified (\%)  & 1.8 & 15.2\\
   Pixels Accepted and Incorrectly Identified (\%)  & 0.2& 1.7\\
    Sample Purity (\%)  & 89.9& 90.2 \\
    \midrule
    High Multiplicity Pixels Accepted and Correctly Identified (\%) & $<0.02$ & 2.2\\
    High Multiplicity Pixels Accepted and Incorrectly Identified (\%) & $<0.02$ & 0.2\\
    Sample Purity (\%) & / & 91.2\\
    \bottomrule
  \end{tabular}
\end{table}
...
The CT5 performance was improved for the new classifier, with 17.9 \% of pixels being correctly identified and accepted, and an accuracy of 66.1\%. Despite this improvement, a clear gap emerged between the between the performance of HESS1 classifier on old telescopes, and the performance of the HESS2 classifier on CT5. Part of this discrepancy can be explained by the HESS2 learning dataset, which has just one DC pixel per event rather than the four HESS1 DC pixels. Additionally, HESS 2 images will have roughly 2600 non-DC pixels per event, versus 3600 DC pixels per event for HESS 1 images. The HESS2 classifier was thus trained on significantly fewer DC pixels, so we would expect to see the clear performance gap in comparison to the HESS1 classifier. In the case of the high-multiplicity events, the gap narrows somewhat. In total 1.9\% of pixels are correctly identified and accepted, although the accuracy rate is only increased to 80.7\%. As before, non-observation of a qualifying high-multiplicity $Q_{DC}$ event allows us to calculate a Poissonian upper limit. Out of 4663 HESS2 images, there were none belonging to events in which the $Q_{DC}$ method identified four DC pixels, limiting the rate to $<0.06\%$. 
...
\begin{table}[h!]
  ...
  \begin{tabular}{ccc}
    ...
    & $Q_{DC}$ & BDT\\
    \midrule
    Pixels Accepted and Correctly Identified (\%) & 5.6 & 17.9 \\
   Pixels Accepted and Incorrectly Identified (\%) & 11.3 & 9.2 \\
    Sample Purity (\%) & 33.0 & 66.1 \\
    \midrule
    High Multiplicity Pixels Accepted and Correctly Identified (\%) & $<0.06$ & 1.9\\
    High Multiplicity Pixels Accepted and Incorrectly Identified (\%) & $<0.06$ & 0.5\\
    Sample Purity (\%) & / & 80.7\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[h!]
  ...
  \begin{tabular}{ccc}
    ...
    & HESS1  & HESS2 \\
    \midrule
    Proton Pixels Accepted (\%) & 15.1 & 58.6\\
    Sample Purity (\%) & 32.2 & 6.8 \\
    \midrule
    High Multiplicity Proton Pixels Accepted (\%) & 2.9 & 21.7\\
    Sample Purity (\%) & 24.5 & 7.7 \\
    \midrule
    Proton Pixels Accepted after All Cuts(\%) & $< 0.02$ & $< 0.07$\\
    Sample Purity (\%) & / & / \\
    \midrule
    Iron Pixels Accepted after All Cuts(\%) & 2.3 & 2.3\\
    Sample Purity (\%) & 89.0 & 79.8 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\caption{The same curves in Figure \ref{fig:corsikalpd1} are shown here for HESS2. Because the CT5 telescope has an area approximately six times larger than the HESS1 telescopes, the LPD is linearly scaled here by a factor of six compared to the HESS1 LPD. It is clear that all other methods of calculating the LPD underestimate the value of $True_{DC}$, as seen in the top plot.}
